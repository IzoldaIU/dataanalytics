...
#hat does the drv variable describe? Read the help for ?mpg to find out. drv variable indicates
# the drive of the vehicle - front, rear, or 4 wheel drive.
...
#What happens if you make a scatterplot of class vs drv. Why is the plot not useful? because both
# variables are categorical and the points overlap. The best practice is to have on categorical and
...
# to control the transparency of datapoints, we assign class to the alpha (aesthetic)
# different point transparency
ggplot(data = mpg) + 
...
# why aren't these datapoints blue? Because the color argument was set within the wrong method,
